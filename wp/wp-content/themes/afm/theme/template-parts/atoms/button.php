...
<<?= $tag ?> class="<?= classNames(
                      'afm-button',
                      $className,
                      'afm-button--size_' . $size,
                      'afm-button--color_' . $color,
                      'afm-button--variant_' . $variant,
                      [
                        'afm-button--display_icon' => !$content && !!($prefix || $postfix),
                        'afm-button--display_block' => !$isInline
                      ]
  ...
  <? if ($prefix) { ?>
    <span class="afm-button__prefix">
      <?= $prefix ?>
    </span>
  <? } ?>
  <? if ($content) { ?>
    <span class="afm-button__content">
      <?= $content ?>
    </span>
  <? } elseif ($is_preview) { ?>
    <span class="afm-button__content" style="opacity: 0.75">
      Fill text field
    </span>
  <? } ?>

  <? if ($postfix) { ?>
    <span class="afm-button__postfix">
      <?= $postfix ?>
    </span>
  <? } ?>
  <? if ($isLoadingBtn) { ?>
    <span class="afm-button__loading">
